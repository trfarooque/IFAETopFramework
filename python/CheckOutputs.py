...
##
def submitFailedJobs( expectedRootFile, scriptFile, batchSystem, batchQueue ):
...
def main(args):
    ...
    if(len(args)<2):
        ...
        print "    python "+args[0]+" input=<name of .chk file> [opt]"
    ...
    for iArg in range(0,len(args)):
        splitted=args[iArg].split("=")
        if(splitted[0]=="input"): 
            inputFile = splitted[1]
        elif(splitted[0]=="relaunch"):
            if(splitted[1].upper()=="TRUE"): 
                relaunchJobs= True
        elif(splitted[0]=="batch"): 
            batchSystem = splitted[1]
        elif(splitted[0]=="queue"): 
            batchQueue = splitted[1]
    if(inputFile==""):
    ...
    for line in f:
        ...
        if hasProblems:
            ...
            if relaunchJobs:
                submitFailedJobs(fileToCheck, scriptFile, batchSystem, batchQueue)
                nRelaunchedJobs += 1
        else:
            nGood += 1
